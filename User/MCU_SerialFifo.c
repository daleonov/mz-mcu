...
void CB_ReadStr(char* sStr, CB_Structure* cbStructure){
	int i;
//	static char sStr[CB_SIZE]="\0"; //?
	for(i=0; i<CB_SIZE; i++)
		sStr[i]=CB_Pop(cbStructure);
//	sprintf (sStr, "Here_is_CB_ReadStr");
//	return sStr;
}
//Функции для отладки.
...
//Выводит содержимое буфера, положения указателей и является ли буфер пустым.
/*
void __CB_Print(CB_Structure* cbStructure){
	int i;
	for(i=0; i<CB_SIZE; i++) printf("%c",cbStructure->sData[i]);
	printf(" Tail:%d, Head:%d, Is Empty: %d\n", cbStructure->nTail, cbStructure->nHead, (unsigned char)cbStructure->bIsEmpty);
}
*/
